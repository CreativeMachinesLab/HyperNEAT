...
namespace NEAT
{
    ...
    nodes(_nodes),
      ...
      parent1md5("none"), //indiactes no 1st parent
      parent2md5("none"), //indiactes no 2nd parent
      thismd5("none"), //indiactes no md5 set
	  order(-1),			//indicates org not yet evaluated in ShapesExperiment
	  parent1Order(-1),		//indiactes no 1st parent
	  parent2Order(-1),		//indiactes no 1st parent
    ...
    {   
        ...
        setThismd5(md5initString.str());

        cout << "creating initial individual with md5: " << getThismd5() << endl;
    ...
    GeneticIndividual::GeneticIndividual(
        ...
        thismd5("none"), //indiactes no md5 set
		order(-1),			//indicates org not yet evaluated in ShapesExperiment
		parent1Order(-1),     //indicates no 1st parent
		parent2Order(-1),     //indicates no 2nd parent
		bestFitnessInLineage(0.0),
		directionChanges(-1), //number of leg direction changes in quadruped experiments
		origFitness(0)
    {
    ...
    //jmc: used when loading from xml files
    GeneticIndividual::GeneticIndividual(TiXmlElement *individualElement):
      ...
      nodesAdded(0),
      linksAdded(0),
      linkWeightsMutated(0),
      linkWeightsNotMutated(0),
      linksDemolished(0)
    {
        bestFitnessInLineage = max(parent1Fitness, parent2Fitness);
        fitness = atof(individualElement->Attribute("Fitness"));
		if(individualElement->Attribute("OrderInPop"))	order = atof(individualElement->Attribute("OrderInPop")); //if order was saved in xml file, grab it
		else {cout << "order was not saved in xml file" << endl;}
    ...
    GeneticIndividual::GeneticIndividual(shared_ptr<GeneticIndividual> parent1,shared_ptr<GeneticIndividual> parent2,bool mate_multipoint_avg, int orderAtBirth)
        :
    fitness(0),
    mutatedAtBirth(false),
    canReproduce(true),
    ...
    parent1md5(parent1->getThismd5()),
    parent2md5(parent2->getThismd5()),
    thismd5("none"),
	order(orderAtBirth),			
	parent1Order(parent1->getOrder()),
	parent2Order(parent2->getOrder()),
	directionChanges(-1), //number of leg direction changes in quadruped experiments
	origFitness(0),
    nodesAdded(0),
    linksAdded(0),
    ...
    {
        cout << "this individual born from " << parent1md5 << " and " << parent2md5 << endl;
      bestFitnessInLineage = max(parent1Fitness, parent2Fitness);
...
//        cout << "leaving GenInd constructor\n";
    ...
    #include <iomanip>
    GeneticIndividual::GeneticIndividual(shared_ptr<GeneticIndividual> parent1,bool tryMutation, int orderAtBirth)
        :
        nodes(parent1->nodes),
        links(parent1->links),
        fitness(0),
        ...
        parent2Fitness(-1.0), //indicates no 2nd parent
        parent1md5(parent1->getThismd5()),
        parent2md5("none"), //indicates no 2nd parent
        thismd5("none"),
		order(orderAtBirth),			
		parent1Order(parent1->getOrder()),
		parent2Order(-1), //indicates no 2nd parent
    ...
    { 
        cout << "this individual born from " << parent1md5 << " only" << endl;
      bestFitnessInLineage = max(parent1Fitness, parent2Fitness);
      if (tryMutation)
        testMutate();
    }
        ...
        links(copy.links)
    {
      ...
      parent1md5 = copy.parent1md5;
      parent2md5 = copy.parent2md5;
      thismd5 = copy.thismd5;
	  order = copy.order;
	  parent1Order = copy.parent1Order;
	  parent2Order = copy.parent2Order;
    }
...
#define DEBUG_ONLY_ONE_MUTATION (0)
        ...
        if (Globals::getSingleton()->getRandom().getRandomDouble()<addNodeProb)
        {
...
#endif
            {
                if (Globals::getSingleton()->getRandom().getRandomDouble()<mutateLinkWeightsProb)
                {
                    ...
                    // cout << "mutateLinkProb:" << mutateLinkProb << endl;
                    for (int a=0;a<(int)links.size();a++)
                    {
                        if ((links[a].getAge()<adultLinkAge)||Globals::getSingleton()->getRandom().getRandomDouble()<mutateLinkProb)
                        {
                            // cout << links[a].getWeight() << " mutated to ";
                            links[a].mutate();
                            mutatedAtBirth = true;                            
                            linkWeightsMutated++;
                            // cout << links[a].getWeight() << endl;
                        ...
                        {
                            // cout << links[a].getWeight() << " not mutated" << endl;
                            linkWeightsNotMutated++;
                        }
                    }
                }
            }
    ...
    void GeneticIndividual::dump(TiXmlElement *root,bool dumpGenes)
    {
        ...
        root->SetAttribute("OrderInPop",order);
	...
	void GeneticIndividual::setOrigFitness(double _origFitness)
			{
				origFitness = _origFitness;
			}
